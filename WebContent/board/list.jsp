...
<table width="700">
    <tr>
         <td align="right" bgcolor="${value_c}">
            <a href="/JSPExam/board/writeForm.do">글쓰기</a>
         </td>
    </tr>
</table>
...
<table width="700" border="1" cellpadding="0" cellspacing="0" align="center">
   ...
   <tr height="30">
       ...
       <a href="/JSPExam/board/content.do?num=${article.num}&pageNum=${currentPage}">
         ${article.subject }</a>
...
<c:if test="${count > 0  }">
	...
	value="${(currentPage-1)/pageBlock }" integerOnly="true" /> 
	 ...
	 <a href="/JSPExam/board/list.do?pageNum=${startPage-pageBlock}">[이전]</a>	
     </c:if>

  <c:forEach var="i" begin="${startPage }" end="${endPage }">
  <a href="/JSPExam/board/list.do?pageNum=${i}">[${i}]</a>
   </c:forEach>

  <c:if test="${endPage < pageCount }">
    <a href="/JSPExam/board/list.do?pageNum=${startPage+pageBlock }">[다음]</a>
  </c:if>
</c:if>
</div>
</body>
